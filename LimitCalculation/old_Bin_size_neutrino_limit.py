...
not using any DM signal. Varying the number of bins used to calculate the limit, and plotting the limit vs Xmax
...
for firstbin in firstbinarr:
    if firstbin == 0:
        lastbinarr =  np.arange(1,51)
    else:
        lastbinarr =  np.arange(5,51)
    for lastbin in lastbinarr:
        ...
        # =============================================================================
        if firstbin == 0:
            limarr_0.append(get_limit_ds20k(plot=False))
        else:
            limarr_4.append(get_limit_ds20k(plot=False))
...
plt.plot(lastbinarr, limarr_4, '-o', linewidth=2, color='firebrick',label=r'$x_{min} = 4e$')
plt.plot(np.arange(1,51), limarr_0, '-o', linewidth=2, color='blue' ,label=r'$x_{min} = 0e$')
